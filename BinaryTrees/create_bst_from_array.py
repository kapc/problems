...
- array has unique integers.
- minimal height so that we create a tree with log(n) height.
...
if __name__ == "__main__":
    test_inputs = [[1], [], [1,2,3,4], [12,34,56,78,90,110,190,1100], [1,1,1,1]]

    for input_arr in test_inputs:
        bst = BST(input_arr)
        assert input_arr == bst.return_result_array()
    ...
    assert bst.check_if_bst()
    while True:
         if not test_input:
             break
         size = len(test_input)
         index = random.randint(0, size - 1)
         bst.delete_node(test_input[index])
         del test_input[index]

    bst = BST([1] * 100)
    for x in range(100):
        bst.delete_node(1)

    test_inputs = [[], [1], [1,2], [1,2,3], [12,34,56,78,90,110,190,1100]]
    for test_input in test_inputs:
        bst = BST(test_input)
        assert bst.check_if_bst(), "Invalid tree %s" %(str(test_input))
        assert bst.is_balanced(), test_input
